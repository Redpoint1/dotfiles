# Generated by Powerlevel10k configuration wizard on 2025-04-01 at 22:53 CEST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      local branch_name=${(V)VCS_STATUS_LOCAL_BRANCH}
      # Extract the part before the first '/' and assign it to part1
      local type="${branch_name%%/*}"

      # Extract the part after the first '/' and assign it to part2, handling the case where '/' may not exist
      ...
      res+="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${type}${branch//\%/%%}"
    fi
